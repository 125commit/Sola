...
2. 截图记账：浏览器对原图计算哈希 → 压缩到 3.5 MB 以内 → `/api/receipts/parse`（线上已启用登录时需会话）→ Qwen-VL 提取全部支出 → Schema 校验 → 多条表单预填 → 用户逐条核对或移除 → 一次写入 IndexedDB。选图热区必须是**直接可点的** `input[type=file]`（不用 label 转发）；`accept` 同时写 MIME 与扩展名。主屏幕 WebView（尤其小米）若拦相册 Intent，提供 `capture` 拍照退路。Service Worker 不缓存 HTML 导航响应。
